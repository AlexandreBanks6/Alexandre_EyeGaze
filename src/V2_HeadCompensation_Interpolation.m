...
for m=[1:num_dir]
    if dirnames{m}(1)=='P' %We have a participant and run calibrations and/evaluations
        pnum=str2double(dirnames{m}(2:end));
        %if pnum==13
        disp(dirnames{m})
        ...
        if (check_calib==true)
            ...
            for i=1:5 %Loops for the other 5 8-point calibrations
                ...
                if (check_calib==true)
                    ...
                    row_train=length(train_cell);

                    empty_ind=[];
                    new_train_cell={};
                    for l=1:row_train
                        if isempty(train_cell{l}{1,2})
                            empty_ind=[empty_ind,l];
                        else
                            new_train_cell=[new_train_cell,train_cell(l)];
                        end
                    end
                    if length(empty_ind)==row_train
                        continue;
                    end
                            
                    model_poly=robustRegressor(new_train_cell); %Get the vanilla model from the initial calibration
                    poly_functions_array{i+1,1}=model_poly;
                    poly_functions_array{i+1,2}=avg_corners;

                end
            end
            %Getting rid of blanks in poly functions array
            empty_inds=[];
            [row_arr,~]=size(poly_functions_array);
            for i=[1:row_arr]
                if isempty(poly_functions_array{i,1})
                    empty_inds=[empty_inds,i];
                end
            end

            poly_functions_array(empty_inds,:)=[];
            ...
            variance_cell_poly=findPOGVariance(data_mat,model_poly_init,dist_cell_init,avg_corners_init,PG_Estimation_Models,max_compensation_models,poly_functions_array,'poly');
            variance_cell_max=findPOGVariance(data_mat,model_poly_init,dist_cell_init,avg_corners_init,PG_Estimation_Models,max_compensation_models,poly_functions_array,'max');
            variance_cell_interp=findPOGVariance(data_mat,model_poly_init,dist_cell_init,avg_corners_init,PG_Estimation_Models,max_compensation_models,poly_functions_array,'interp');
            %variance_cell={};
            ...
            [mean_accuracies,total_results]=evalModelsRegressComp(data_mat,model_poly_init,dist_cell_init,avg_corners_init,PG_Estimation_Models,max_compensation_models,poly_functions_array,variance_cell_poly,variance_cell_max,variance_cell_interp);
...
function [mean_accuracies,total_results]=evalModelsRegressComp(data_mat,model_cell,dist_cell,avg_corners,PG_Estimation_Models,max_compensation_models,poly_functions_array,variance_cell_poly,variance_cell_max,variance_cell_interp)
    %{
    Inputs:
    ...
    total_results=evalAccuracyComp(model_cell,reformatted_data,right_headers,left_headers,check_model_right,check_model_left,dist_cell,avg_corners,PG_Estimation_Models,max_compensation_models,poly_functions_array,variance_cell_poly,variance_cell_max,variance_cell_interp);
...
function total_results=evalAccuracyComp(model_cell,reformatted_data,right_headers,left_headers,check_model_right,check_model_left,dist_cell,avg_corners,PG_Estimation_Models,max_compensation_models,poly_functions_array,variance_cell_poly,variance_cell_max,variance_cell_interp)
   %{
    Inputs:
    reformatted_data:
    ...
    for i=[1:row_n]
                ...
                if length(valid_header)>2
                    ...
                    if ~isnan(d_calib) && ~isnan(d_curr)
                        ...
                        if ~all(isnan(weighted_poly_x)) && ~all(isnan(weighted_poly_y))
                            ...
                            %}
                            accuracy_interp_right=getAccuracy_mm(t_x,t_y,POG_x_interp_right,POG_y_interp_right);
                ...
                if length(valid_header)>2
                    ...
                    if ~isnan(d_calib) && ~isnan(d_curr)
                        ...
                        %Finding interpolation results
                            ...
                            %}
                            accuracy_interp_left=getAccuracy_mm(t_x,t_y,POG_x_interp_left,POG_y_interp_left);
        ...
        if exist('POG_x_interp_right','var') && exist('POG_y_interp_right','var') && exist('POG_x_interp_left','var') && exist('POG_y_interp_left','var') 
           %Getting variance of corresponding POGs (var_right_x,
            ...
            [POG_combined_x,POG_combined_y]=findCombinedWithVariance(variance_cell_interp,POG_x_interp_right,POG_x_interp_left,POG_y_interp_right,POG_y_interp_left);          
...
function variance_cell=findPOGVariance(data_mat,model_cell,dist_cell,avg_corners,PG_Estimation_Models,max_compensation_models,poly_functions_array,variance_type)
...
%}
    ...
    %left_x, left_y)
    POG_Results=evalAccuracyCompVariance(model_cell,reformatted_data,right_headers,left_headers,check_model_right,check_model_left,dist_cell,avg_corners,PG_Estimation_Models,max_compensation_models,poly_functions_array,variance_type);
...
function total_results=evalAccuracyCompVariance(model_cell,reformatted_data,right_headers,left_headers,check_model_right,check_model_left,dist_cell,avg_corners,PG_Estimation_Models,max_compensation_models,poly_functions_array,variance_type)
   %{
    Inputs:
    reformatted_data:
    frame_no, pg0_rightx, pg0_righty, ..., pg2_rightx, pg2_righty, pg0_leftx, pg0_lefty,..., pg2_leftx, pg2_lefty,
...
%                                     results_row(2)=abs(t_y-POG_y_max_right);
                    ...
                    if ~isnan(d_calib) && ~isnan(d_curr)
                        ...
                        if ~all(isnan(weighted_poly_x)) && ~all(isnan(weighted_poly_y))
                            POG_x_interp_right=findPOG(weighted_poly_x',predictors_x);
                            POG_y_interp_right=findPOG(weighted_poly_y',predictors_y);
                            if strcmp(variance_type,'interp')
                            results_row(1)=POG_x_interp_right;
                            results_row(2)=POG_y_interp_right;
                            %results_row(1)=abs(t_x-POG_x_interp_right);
                            %results_row(2)=abs(t_y-POG_y_interp_right);
                            end
                        end
                    end       
...
%                                     results_row(4)=abs(t_y-POG_y_max_left);
                    ...
                    if ~isnan(d_calib) && ~isnan(d_curr)
                        ...
                        %Finding interpolation results
                            ...
                            %results_row(3)=abs(t_x-POG_x_interp_left);
                            %results_row(4)=abs(t_y-POG_y_interp_left);
                            end
