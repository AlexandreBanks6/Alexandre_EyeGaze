...
comp_data_right(:,[1,3,5])=(comp_data_right(:,[1,3,5])./100).*WIDTH_CONVERSION;
comp_data_right(:,[2,4,6])=(comp_data_right(:,[2,4,6])./100).*HEIGHT_CONVERSION;
...
hold on
showBestFitLine(inner_x,del_pog,0.1);
hold off
...
hold on
showBestFitLine(inner_y,del_pog,0.1);
hold off 
...
hold on
showBestFitLine(outer_x,del_pog,0.1);
hold off 
...
hold on
showBestFitLine(outer_y,del_pog,0.1);
hold off 
...
% Left label
ylabel(t,['\deltaPOG ',del_pog_direction,' (mm)'],'FontName','Times New Roman','FontSize',15,'Color','k','FontWeight','bold')
