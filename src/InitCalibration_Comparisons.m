...
function [reformatted_data_right,reformatted_data_left]=reformatDataEval(eval_data)
    ...
    % reformatted_data_right=frame_no,pg0_leftx,pg0_lefty,pg1_leftx,pg1_lefty,target_x,target_y
...
function [accuracy_results]=evalAccuracy(model_cell,reformatted_data,header,type)
    ...
    for i=[1:row_n]
        ...
        if length(nan_indx_values)<5 %At least one x,y pair are detected
            ...
            cur_ind=1;
            for j=[1:2:row_new]
                if (updated_model_cell{j,2}<cur_val) && (strcmp(type,'robust'))
            ...
            POG_x=model_x(1)+model_x(2)*pg_x.^2+model_x(3)*pg_x.*pg_y+model_x(4)*pg_y.^2+...
                model_x(5)*pg_x+model_x(6)*pg_y;
            POG_y=model_y(1)+model_y(2)*pg_x.^2+model_y(3)*pg_x.*pg_y+model_y(4)*pg_y.^2+...
                model_y(5)*pg_x+model_y(6)*pg_y;
            t_x=curr_row(8);
            t_y=curr_row(9);
...
function [accuracy_results]=evalAccuracyCombined(model_cell,right_data,left_data,header_right,header_left,type)
    ...
    for i=[1:row_n]
        ...
        if length(nan_indx_values)<5 %At least one x,y pair are detected
            ...
            pg_x=pgsonly(pg_x_ind);
            pg_y=pgsonly(pg_y_ind);
            POG_x_right=model_x(1)+model_x(2)*pg_x.^2+model_x(3)*pg_x.*pg_y+model_x(4)*pg_y.^2+...
                model_x(5)*pg_x+model_x(6)*pg_y;
            POG_y_right=model_y(1)+model_y(2)*pg_x.^2+model_y(3)*pg_x.*pg_y+model_y(4)*pg_y.^2+...
                model_y(5)*pg_x+model_y(6)*pg_y;
        ...
        if length(nan_indx_values)<5 %At least one x,y pair are detected
            ...
            pg_x=pgsonly(pg_x_ind);
            pg_y=pgsonly(pg_y_ind);
            POG_x_left=model_x(1)+model_x(2)*pg_x.^2+model_x(3)*pg_x.*pg_y+model_x(4)*pg_y.^2+...
                model_x(5)*pg_x+model_x(6)*pg_y;
            POG_y_left=model_y(1)+model_y(2)*pg_x.^2+model_y(3)*pg_x.*pg_y+model_y(4)*pg_y.^2+...
                model_y(5)*pg_x+model_y(6)*pg_y;
        else
            continue
        end
