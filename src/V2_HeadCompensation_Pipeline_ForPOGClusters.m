...
for m=[1:num_dir]
    if dirnames{m}(1)=='P' %We have a participant and run calibrations and/evaluations
        ...
        if strcmp(dirnames{m},'P08') %||strcmp(dirnames{m},'P23')||strcmp(dirnames{m},'P24')||strcmp(dirnames{m},'P25')||strcmp(dirnames{m},'P26')||strcmp(dirnames{m},'P27')||strcmp(dirnames{m},'P28')
            disp(dirnames{m});
            %Reading In Data
            ...
            if (check_calib==true)
                ...
                %data_mat=readmatrix([data_root,'/',dirnames{m},'/calib_only_merged_Eval_Lift1_9Point.csv']);
%                 bool_check=checkEvalData(eval_lift1_9dot);
%                 if ~bool_check
%                     eval_lift1_9dot=[];
%                 end
                %data_mat=readmatrix([data_root,'/',dirnames{m},'/calib_only_merged_Eval_Lift2_9Point.csv']);
%                 bool_check=checkEvalData(eval_lift2_9dot);
%                 if ~bool_check
%                     eval_lift2_9dot=[];
%                 end
                data_mat=readmatrix([data_root,'/',dirnames{m},'/calib_only_merged_Eval_Lift3_9Point.csv']);
...
%% Plotting Clusters
model_based_results=total_results(:,[5,6]);

push_left_inds=model_based_results(:,1)<40;
push_right_inds=model_based_results(:,1)>60;
model_based_results(push_left_inds,1)=model_based_results(push_left_inds,1)-10;
model_based_results(push_right_inds,1)=model_based_results(push_right_inds,1)+10;


model_based_results(:,2)=100-model_based_results(:,2);

poly_results=total_results(:,[1,2]);

push_left_inds=poly_results(:,1)<40;
push_right_inds=poly_results(:,1)>60;
poly_results(push_left_inds,1)=poly_results(push_left_inds,1)-10;
poly_results(push_right_inds,1)=poly_results(push_right_inds,1)+10;
poly_results(:,2)=100-poly_results(:,2);

max_results=total_results(:,[3,4]);
push_left_inds=max_results(:,1)<40;
push_right_inds=max_results(:,1)>60;
max_results(push_left_inds,1)=max_results(push_left_inds,1)-10;
max_results(push_right_inds,1)=max_results(push_right_inds,1)+10;
max_results(:,2)=100-max_results(:,2);

%Getting targets:
targets=total_results(:,[7,8]);
[~,ia,~]=unique(targets,'rows');
targets=targets(ia,:);

push_left_inds=targets(:,1)<40;
push_right_inds=targets(:,1)>60;
targets(push_left_inds,1)=targets(push_left_inds,1)-10;
targets(push_right_inds,1)=targets(push_right_inds,1)+10;
targets(:,2)=100-targets(:,2);

%Adjusting sizing
targets(:,1)=(targets(:,1)./100)*284.48;
targets(:,2)=(targets(:,2)./100)*213.36;

model_based_results(:,1)=(model_based_results(:,1)./100)*284.48;
model_based_results(:,2)=(model_based_results(:,2)./100)*213.36;

poly_results(:,1)=(poly_results(:,1)./100)*284.48;
poly_results(:,2)=(poly_results(:,2)./100)*213.36;

max_results(:,1)=(max_results(:,1)./100)*284.48;
max_results(:,2)=(max_results(:,2)./100)*213.36;

poly_color='dr';
max_color='xg';
model_color='ob';
figure;
plot(poly_results(:,1),poly_results(:,2),poly_color,max_results(:,1),...
    max_results(:,2),max_color,model_based_results(:,1),model_based_results(:,2),model_color);
hold on
patchcolor='b';
for i=[1:9]
    dash_type='-';
                ...
                if length(valid_header)>2
                    ...
                    if ~isnan(d_calib) && ~isnan(d_curr)
                        ...
                        if length(comp_model_y_right)>0 && ~any(isnan(predictors_y))
                                ...
                                %results_row(4)=del_POG_y_tree;
                                POG_y_tree_right=del_POG_y_tree+POG_y_poly_right;
                                %results_row(4)=POG_y_tree_right;
            ...
            if length(nan_indx_values)<3 %At least two x,y pairs are detected
                ...
                if length(valid_header)>2
                    ...
                    if ~isnan(d_calib) && ~isnan(d_curr)
                        ...
                        results_row(1)=POG_x_poly_left;
                        results_row(2)=POG_y_poly_left;
